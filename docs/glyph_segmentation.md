...
segmentations is an area of active development in the ift encoder. The current implementation
can produce segmentations that satisfy the closure requirement, but does not yet necessarily produce
ones that are performant.

This document captures how the current prototype implementation works, but several areas remain
that need improvement and/or further development:
